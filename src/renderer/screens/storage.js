...
(function () {
  ...
  SP.screens.storagecat = function (host) {
    ...
    host.appendChild(el('div', { style: 'height:14px;border-radius:7px;background:var(--track);overflow:hidden;margin-bottom:8px' }, [
      el('span', { style: 'display:block;height:100%;background:' + color + ';width:' + (used ? (c.bytes / used * 100).toFixed(1) : 0) + '%' })
    ]));
    host.appendChild(el('div', { style: 'font-size:12.5px;color:var(--text-3);margin-bottom:24px', text: used ? (c.bytes / used * 100).toFixed(0) + '% of your used space' : '' }));

    host.appendChild(el('div', { style: 'display:flex;align-items:center;gap:13px;padding:18px 20px;border-radius:14px;background:var(--panel);border:1px solid var(--border);color:var(--text-2)' }, [
      ...
      el('div', { style: 'font-size:13.5px;line-height:1.55' }, ['Spaci measures this category as a whole. To act on individual files, open Large Files or the relevant cleaner. The reclaimable parts are surfaced in Recommendations.'])
    ]));
  };
})();
